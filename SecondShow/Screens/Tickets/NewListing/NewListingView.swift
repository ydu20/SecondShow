...
struct NewListingView: View {
                    ...
                    Button {
                        ...
                        for eventDate in vm.eventDates {
                            if let eventInd = eventDate.events.firstIndex(where: {$0.name == eventName}) {
                                ...
                                if (eventDate.events[eventInd].date != dateFormatter.string(from: self.eventDate)) {
                                    notifyUser("Error: an event with the same name already exists on \(eventDate.events[eventInd].date)", Color(.systemRed))
                                    return
                                }
                        ...
                        if (newMaxListingNum == 1) {
                            ...
                            eventDoc.setData(eventData) { err in
                                ...
                                addEventListing(eventDoc: eventDoc, eventData: eventData, listingNumber: newMaxListingNum)
                            }
                        } else {
    ...
    private func addEventListing(eventDoc: DocumentReference, eventData: [String: Any], listingNumber: Int) {
